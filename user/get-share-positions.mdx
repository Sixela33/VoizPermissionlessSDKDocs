...
Inventory, remaining cost, and realized PnL come from that table. Mark price comes from the indexer’s outcome row (0 or 1 after resolve). Raw fills stay on `trades()`. AMM LP lots stay on `lpPositions()`.
...
You can filter with `markets` (addresses or summaries). Summaries also supply labels, tokens, and resolution. Inventory still comes from the indexer.
